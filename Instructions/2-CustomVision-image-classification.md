...
1. In Visual Studio Code, view the training images in the **2-image-classification/training-images** folder where you downloaded the repository. This folder contains subfolders of apple, banana, and orange images.
2. In a new browser tab, open the Custom Vision portal at `https://customvision.ai`. **Sign in** using the Microsoft account associated with your Azure subscription and agree to the terms of service.
...
1. In Visual Studio Code, in the **Explorer** pane, browse to the **2-image_classification** folder and expand the **Python** folder.
2. Right-click the **train-classifier** folder and **open in integrated terminal**. Then install the Custom Vision Training package by running the appropriate command for your language preference:
...
1. In Visual Studio Code, in the **2-image-classification** folder,  right- the **test-classifier** folder and **open in integrated terminal**. Then enter the following SDK-specific command to install the Custom Vision Prediction package:
...
3. Open the configuration file for your client application (*.env* for Python) and update the configuration values it contains to reflect the endpoint and key for your Custom Vision *prediction* resource, the project ID of the classification project (used for training app, *.env* file), and the name of your published model (which should be *fruit-classifier*). Save your changes.
